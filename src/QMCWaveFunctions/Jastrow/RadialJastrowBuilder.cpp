...
bool RadialJastrowBuilder::put(xmlNodePtr cur)
{
  ...
  if (TypeOpt.find("one") < TypeOpt.size())
  {
    ...
    else if (Jastfunction.find("gauss") != std::string::npos)
    {
      success = createJ1<GaussianFunctor<RealType>>(cur);
    }
    else
    {
  ...
  else if (TypeOpt.find("two") < TypeOpt.size())
  {
    ...
    else if (Jastfunction.find("mcmillan") != std::string::npos)
    {
      success = createJ2<McMillanFunctor_MS<RealType>>(cur);
    }
    else
    {
